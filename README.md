...
3. Saya juga telah mencoba menggunakan :contentReference[oaicite:0]{index=0} untuk menguji aplikasi. Tool ini sangat membantu karena memungkinkan saya untuk mengirim HTTP request (seperti GET, POST, DELETE) dengan mudah tanpa harus membuat frontend terlebih dahulu. Beberapa fitur yang menurut saya sangat berguna adalah fitur collection untuk menyimpan request, environment untuk mengatur variabel seperti URL atau token, serta tampilan response yang jelas sehingga memudahkan debugging. Postman sangat membantu dalam memastikan endpoint yang saya buat sudah berjalan dengan benar dan akan sangat berguna juga untuk project kelompok maupun pengembangan software di masa depan.
